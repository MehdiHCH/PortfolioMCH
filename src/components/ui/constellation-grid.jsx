/**
 * ConstellationGrid — faithful port of the supplied component.
 *
 * Converted from .tsx to .jsx because this project is JavaScript (no tsconfig).
 * Behaviour is otherwise unchanged: window-sized opaque canvas, spring-mass
 * node grid, cursor shockwaves, radar rings and hex coordinate readouts.
 *
 * This is the standalone full-screen treatment. The Projects section uses
 * `@/components/ProjectsConstellation`, which is the same idea reworked into a
 * transparent, container-sized background layer.
 */
...
export default function ConstellationGrid() {
  ...
  useEffect(() => {
    ...
    const mouse = {
      x: -1000, y: -1000, prevX: -1000, prevY: -1000, vx: 0, vy: 0, radius: 220,
    };
    ...
    const handleResize = () => {
      ...
      // compounds, which shrinks the drawing on every subsequent resize.
    ...
    const render = (now) => {
      ...
      const accentColor = isDarkMode ? "155, 126, 249" : "109, 40, 217"; // site violet
